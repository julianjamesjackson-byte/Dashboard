...
export default function Dashboard() {
  ...
  const fetchN8nData = async () => {
    ...
    try {
      ...
      // n8n sends: [{ success, dashboard: {...}, supportingData: {...}, campaignStatus: [...], updatedAt }]
      const item = Array.isArray(rawData) ? rawData[0] : rawData;
      const d = item.dashboard || {};
      const s = item.supportingData || {};
      ...
      // Format percentage (e.g. 45.2 -> "45.2%", 0 -> "0%")
      const fmtPct = (val) => {
        const num = Number(val);
        if (isNaN(num)) return String(val);
        return num + '%';
      ...
      const kpis = {
        emailsSent: fmt(d.totalEmailsSent ?? 0),
        activeLeads: fmt(d.activeLeads ?? s.totalProspects ?? 0),
        openRate: fmtPct(d.openRate ?? 0),
        replies: fmt(d.totalReplies ?? 0),
        positiveReplyRate: fmtPct(d.positiveReplyRate ?? 0),
        meetingsBooked: fmt(d.meetingsBooked ?? 0),
        bounceRate: fmtPct(d.bounceRate ?? 0),
      };
